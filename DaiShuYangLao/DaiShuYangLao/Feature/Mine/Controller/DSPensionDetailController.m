...
//#import "UIButton+DSNavigationTitle.h"
#import "DSPensionDetailDropDownMenu.h"


@interface DSPensionDetailController ()<UITableViewDelegate,UITableViewDataSource>{
...
static CGFloat const kDropDownMenuHeight = 160;

#define kPageSize (boundsHeight>667? 20:10)
//static NSInteger const kPageSize = 20;

@implementation DSPensionDetailController
...
//    self.re
   ...
   __weak typeof (self)weakSelf = self;
    ...
    _dropMenuView.dataArray = @[@"全部",@"支出",@"收入",@"提现"];
    _dropMenuView.clickAtIndexPathBlock = ^(NSIndexPath *indexPath) {
        NSString * title = @[@"养老备用金明细",@"支出明细",@"收入明细",@"提现明细"][indexPath.row];
...
- (UIButton *)navigationButton{
    if (!_navigationButton) {
        ...
        [_navigationButton setTitle:@"养老备用金明细" forState:UIControlStateNormal];
...
- (void)reqeustDataWithRefreshFlag:(DSRefreshFlag)refreshFlag{
    ...
    [DSHttpResponseData minePensionDetailListGetByParams:params callback:^(id info, BOOL succeed, id extraInfo) {
